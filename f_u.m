...
%     the negative of the normal vector
    n_aux = sign(D(D(:,4)==2,1)).*n(D(D(:,4)==2,3),:);
    v = P.r-S(D(:,2),:);
    dist = D(:,1);
 
%  flip the final vector, so we have the distance from the domain point to the mesh   
    f_u(D(:,4) == 1,:) = dH(dist(D(:,4) == 1),P.alpha,P.beta)...
            .*v(D(:,4) == 1,:)./abs(dist(D(:,4) == 1));
    f_u(D(:,4) == 2,:) = dH(dist(D(:,4) == 2),P.alpha,P.beta)...
            .*n_aux.*sign(dot(n_aux,v(D(:,4) == 2,:),2));
    f_d(D(:,4) == 1,:) = -v(D(:,4) == 1,:)./abs(dist(D(:,4) == 1));
    f_d(D(:,4) == 2,:) = -n_aux.*sign(dot(n_aux,v(D(:,4) == 2,:),2));
