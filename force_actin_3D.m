...
%  get the contribution of the force to the vertex       
        ...
        if ~isempty(r_T_index_s1)
            aux_T1  = (P.delta_x^3).*sum(dH(D(r_T_index_s1(:,1),1),alpha,beta).*...
                B(r_T_index_s1(:,1)).*sign(dot(v(r_T_index_s1(:,1),:),sign(D(r_T_index_s1(:,1),1)).*n(r_T_index_s1(:,2),:),2)).*...
                (sign(D(r_T_index_s1(:,1),1)).*n(r_T_index_s1(:,2),:) + ...
                (cross(v(r_T_index_s1(:,1),:)-dot(sign(D(r_T_index_s1(:,1),1)).*n(r_T_index_s1(:,2),:),...
                v(r_T_index_s1(:,1),:),2).*sign(D(r_T_index_s1(:,1),1)).*n(r_T_index_s1(:,2),:),...
                sign(D(r_T_index_s1(:,1),1)).*ds1(r_T_index_s1(:,2),:)))./N_length(r_T_index_s1(:,2),:)));
        end
            
        aux_T2 = zeros(1,3);
        if ~isempty(r_T_index_s2)
            aux_T2 =  (P.delta_x^3).*sum(dH(D(r_T_index_s2(:,1),1),alpha,beta).*...
                B(r_T_index_s2(:,1)).*sign(dot(v(r_T_index_s2(:,1),:),sign(D(r_T_index_s2(:,1),1)).*n(r_T_index_s2(:,2),:),2)).*...
                (cross(v(r_T_index_s2(:,1),:)-dot(sign(D(r_T_index_s2(:,1),1)).*n(r_T_index_s2(:,2),:),...
                v(r_T_index_s2(:,1),:),2).*sign(D(r_T_index_s2(:,1),1)).*n(r_T_index_s2(:,2),:),...
                sign(D(r_T_index_s2(:,1),1)).*ds2(r_T_index_s2(:,2),:))./N_length(r_T_index_s2(:,2),:)));
        end
        
        aux_T3 = zeros(1,3);
        if ~isempty(r_T_index_s3)
            aux_T3 =  (P.delta_x^3).*sum(dH(D(r_T_index_s3(:,1),1),alpha,beta).*...
                B(r_T_index_s3(:,1)).*sign(dot(v(r_T_index_s3(:,1),:),sign(D(r_T_index_s3(:,1),1)).*n(r_T_index_s3(:,2),:),2)).*...
                (cross(v(r_T_index_s3(:,1),:)-dot(sign(D(r_T_index_s3(:,1),1)).*n(r_T_index_s3(:,2),:),...
                v(r_T_index_s3(:,1),:),2).*sign(D(r_T_index_s3(:,1),1)).*n(r_T_index_s3(:,2),:),...
                sign(D(r_T_index_s3(:,1),1)).*ds3(r_T_index_s3(:,2),:))./N_length(r_T_index_s3(:,2),:)));
        end
